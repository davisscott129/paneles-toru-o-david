...
with tab2:
    if not run:
        ...
        if not res['es_optimo']:
            ...
            with col_chart:
                st.markdown("**📊 Distribución de Paneles**")
                df_bar = pd.DataFrame({
                    'Paneles': [res['xa'], res['xb'], res['xc']],
                }, index=['Panel A (400W)', 'Panel B (450W)', 'Panel C (550W)'])
                st.bar_chart(df_bar, color='#10b981', use_container_width=True)
...
with tab3:
    if not run:
        ...
        if res.get('es_optimo'):
            ...
            st.markdown("**📈 Ahorro Acumulado vs. Inversión a 20 años**")
            st.line_chart(df_linea, color=['#10b981', '#f59e0b'], use_container_width=True)
...
with tab4:
    ...
    with col_a:
        st.markdown("**💰 Inversión Óptima por Casa (USD)**")
        df_inv = pd.DataFrame({
            'Inversión (USD)': [resolver_casa(cid)['inversion'] for cid in CASAS],
        }, index=[f"Casa {cid}" for cid in CASAS])
        st.bar_chart(df_inv, color='#059669', use_container_width=True)

    with col_b:
        st.markdown("**⚡ Generación vs Demanda Diaria (kWh)**")
        df_gen = pd.DataFrame({
            'Generación (kWh)': [resolver_casa(cid)['generacion_diaria'] for cid in CASAS],
            'Demanda (kWh)':    [CASAS[cid]['consumo_diario'] for cid in CASAS],
        }, index=[f"Casa {cid}" for cid in CASAS])
        st.bar_chart(df_gen, color=['#10b981', '#f59e0b'], use_container_width=True)
